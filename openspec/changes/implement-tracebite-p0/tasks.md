...
- [ ] 3.4 撰寫並執行第 3 階段資料庫儲存與查詢邏輯的單元測試 (Unit Test)，確保測試合格後才可繼續
...
- [ ] 4.5 撰寫並執行第 4 階段 Agent 的單元測試 (Unit Test)（若需 API 金鑰，必須讀取 `.env` 並指引使用者設定），確保測試合格後才可繼續
...
- [ ] 5.5 撰寫並執行第 5 階段後端 API 端點的單元測試 (Unit Test)，確保測試合格後才可繼續
